...
test('Ao calcular o frete deve retornar valor', async () => {
	...
	const response = await request(app.httpServer.app).post('/calculate/each-shipping')
		.send({
	...
	expect(response.body.shippings).toContainEqual({id: 1, shipping: 94.68});
	expect(response.body.shippings).toContainEqual({id: 2, shipping: 17.05});
});
