import {CalculateEachShipping} from '../../../src/application/usecases/CalculateEachShipping';
...
test('Deve calcular frete com 1 item', async () => {
	...
	const shippingPrice = await calculateEachShipping.execute({
		destination: '59700-970',
		orderItems: [
			{id: 1, dimensions: guitar.dimensions, weight: guitar.weight, origin: guitar.addressCep, quantity: 1},
		],
	});

	expect(shippingPrice).toContainEqual({id: 1, shipping: 47.34});
});
...
test('Deve calcular frete com 2 do mesmo item', async () => {
	...
	const shippingPrice = await calculateEachShipping.execute({
		destination: '59700-970',
		orderItems: [
			{id: 1, dimensions: guitar.dimensions, weight: guitar.weight, origin: guitar.addressCep, quantity: 2},
		],
	});

	expect(shippingPrice).toContainEqual({id: 1, shipping: 94.68});
});
...
test('Deve calcular frete com 2 itens', async () => {
	...
	const shippingPrice = await calculateEachShipping.execute({
		...
		orderItems: [
			{id: 1, dimensions: guitar.dimensions, weight: guitar.weight, origin: guitar.addressCep, quantity: 2},
			{id: 2, dimensions: amp.dimensions, weight: amp.weight, origin: amp.addressCep, quantity: 1},
		],
	});

	expect(shippingPrice).toContainEqual({id: 1, shipping: 94.68});
	expect(shippingPrice).toContainEqual({id: 2, shipping: 17.05});
});
